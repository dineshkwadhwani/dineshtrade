...
- Backtest entry replay now enforces the same strategy-affecting gate family as live auto execution where historical data exists: full per-trade sizing with all-or-skip cash checks, buy/sell day quotas, sector concentration, pyramiding, panic-sell, and benchmark-based market gates. Historical GIFT-Nifty is approximated from the benchmark's opening gap vs prior close; intraday circuit uses benchmark 5-minute candles.
...
- Live Strategy 2 monitoring now combines `/quote` LTP with the latest completed Kite 5-minute candle per open position. This closes the gap between cron ticks: if the current LTP has not reached T1 or T2 but the last completed candle's high already did, the monitor still sends the market SELL immediately so valid intraday target touches are not missed between cron runs. The candle-high fallback only applies when that completed candle is not older than the lot being evaluated, and it no longer bypasses the no-loss gate when the current sell price is below that lot's own entry.
