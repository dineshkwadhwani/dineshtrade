...
- Settings now has a dedicated **Backtest** tab beside **Strategies**. It loads all saved strategies into a dropdown, accepts a trading-day lookback, runs the authenticated backtest API, and renders summary, trades, and equity curve on the same page. Dip strategies replay from daily candles; momentum strategies replay from 5-minute candles with same-page results. The trade table now shows a `Charges` column, the main summary tiles use net-after-charges values, and every completed run is persisted into `~/dineshtrade/data/backtest-history.json` so it can be sorted, reloaded, reset, or analysed later from the same tab.
- Settings → Strategies now starts with two collapsed accordions: **Fixed Rules · read only** and **Shared Capital · applies to all strategies**. The old duplicated fixed-rules block was removed from the General tab. A new **Export to CSV** action downloads the current draft config as `Strategy name`, `Parameter`, `Parameter description`, `Value` rows, with `Fixed Rules` used as the strategy name for the read-only section.
- Backtest History analysis uses the app's configured AI provider from `.env` through `lib/ai.ts`, not a hard-coded vendor. The analysis call is only made after user confirmation and is blocked until at least 3 saved runs exist.
- Real trade report (`lib/tradeReport.ts`) — reconstructs actual journaled BUY / SELL legs for a selected date range, with optional account and strategy filters (including Manual). Carry-in positions remain linked to the original BUY, and open rows are marked at the selected To date. The page now deliberately avoids backtest-style equity-curve / drawdown presentation and instead focuses on broker-truth executions plus estimated net/charge overlays where exact historical broker charge ledgers are unavailable. Exposed via `POST /api/trade-report` and rendered on the top-level `/trade-report` page.
...
- **Deployable-cap denominator** — Engine `/api/capital` now computes deployable cap from total live capital, not cash alone: `totalCapital = deployed + available`, `maxDeployable = totalCapital × maxDeployPct`, `reserve = totalCapital − maxDeployable`. This matches the business rule that the configured deployable % applies to the full account capital pool.
- **Capital reconciliation strip** — Engine `/api/capital` now exposes `reconciliationBase` (optional per-account from `config/accounts.json`), `livePnl`, and `reconciliationResidual = liveCapital − (reconciliationBase + livePnl)`. The capital tile shows this residual as `Ledger Adjustment` so small broker-side deltas are visible instead of appearing as a defect in MTM math.
- **Engine capital tile breakdown** — Trading Engine `CapitalBar` now also shows `Net Realized P&L`, `Net Unrealized MTM`, `Net Live P&L`, and `Live Capital`. These numbers are sourced from the same journal-backed trade-report charge model used by `/trade-report`, so the Engine page can serve as the live MTM source of truth without cross-checking another page.
...
- Cron starts from `server.js`, which loads a build-time bundled `dist/cron-runtime.cjs`, so the PM2-owned Node process is the sole scheduler owner without any runtime TS loader hooks. `instrumentation.ts` remains a no-op compatibility hook.
