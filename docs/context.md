...
- Real trade report (`lib/tradeReport.ts`) — reconstructs actual journaled BUY / SELL legs into the same `summary + trades + equityCurve` shape as the backtest report. It supports a From / To date range, optional account and strategy filters (including Manual), carries pre-range open positions forward so in-range exits stay linked to the original BUY row, marks open rows at the selected To date, and now applies the same estimated Zerodha-style charge model used by backtests so the report can show gross profit, brokerage, net profit, and net MTM tiles. Summary capital and return percentages use the live total capital base for the selected account scope (`available + deployed`) instead of just the traded notional inside the selected range. Exposed via `POST /api/trade-report` and rendered on the new top-level `/trade-report` page.
